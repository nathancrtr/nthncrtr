...
|---|---|---|---|---|
| **natto** | `natto` | Hub | Beelink Mini S12, x86_64, Ubuntu Server 26.04 LTS (migrated from Raspberry Pi/arm64/Debian on 2026-05-16 — see `runbooks/migrate-natto.md` Gaps §"2026-05-16") | Caddy (native, systemd), Pi-hole, Navidrome, Homepage, qBittorrent (behind Gluetun + Proton VPN), the *arrs, Nextcloud (Tailscale-only), Jellyfin (**the one internet-exposed service** — public for trusted users via a **Cloudflare Tunnel** (`services/cloudflared`, outbound; GFiber can't port-forward); brute-force handled by a Cloudflare WAF Rate-Limiting rule (dashboard, not in repo); inside clients use Caddy + Pi-hole split-horizon — see `services/jellyfin/README.md` and WORKLIST 6.6), Authelia (SSO gate for the web-admin tier via Caddy `forward_auth` — fronts the *arrs/qBittorrent/Homepage only; **not** Jellyfin, which would break its native clients), Immich (self-hosted Google Photos replacement at `photos.nthncrtr.com` — tailnet-only like Nextcloud, **not** internet-exposed and deliberately **not** behind Authelia: its native mobile backup app breaks behind `forward_auth`, same as Jellyfin; library + postgres on `/srv` ext4, not exfat — see `services/immich/README.md`) — all docker-managed compose projects. SMB/Samba is **not a supported feature** here (the old `\\natto\Music` share was dropped at the migration and is intentionally not reproduced). |
...
**New-subdomain gotcha — Pi-hole negative cache.** Inside clients resolve via Pi-hole, which *forwards* `*.nthncrtr.com` upstream (so they get the Tailscale IP) — **except** `music.nthncrtr.com` and `play.nthncrtr.com`, which have Pi-hole local-DNS overrides → natto's LAN IP `192.168.1.50` (`/etc/pihole/hosts/custom.list`, mirrored in `pihole.toml` `hosts[]`; LAN-direct, no Tailscale hop). If a new name was queried *before* its Cloudflare record existed (diagnosing, a browser, etc.), Pi-hole negative-caches the `NXDOMAIN` for the zone's SOA-minimum TTL (**1800s / 30 min**). Symptom: the record is correct at `1.1.1.1`/`8.8.8.8`/Cloudflare's NS but Pi-hole still returns `NXDOMAIN`. Fix: **wait it out** (the SOA TTL counts down — `dig +noall +authority <name> @192.168.1.240`); do **not** restart Pi-hole to force it (safety rule 1 — a household DNS outage to dodge a ≤30-min wait is a bad trade). Clients also negative-cache locally (macOS: `sudo dscacheutil -flushcache; sudo killall -HUP mDNSResponder`; browsers cache separately). Add a Pi-hole local override only if you specifically want LAN-direct like `music`/`play`.
