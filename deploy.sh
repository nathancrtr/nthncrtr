...
deploy_qbittorrent() {
  ...
  install_file "$REPO_ROOT/services/qbittorrent/qbit-keep-only-complete.sh" /srv/qbittorrent/qbit-keep-only-complete.sh 0755
  install_file "$REPO_ROOT/services/qbittorrent/qbit-keep-only-complete.py" /srv/qbittorrent/qbit-keep-only-complete.py 0755
...
deploy_bazarr() {
  ...
  local CHANGED=0
  (( DRY_RUN )) || {
    [[ -d /srv/bazarr ]]        || { install -d -o nthncrtr -g nthncrtr -m 0755 /srv/bazarr;        note "created /srv/bazarr"; }
    [[ -d /srv/bazarr/config ]] || { install -d -o nthncrtr -g nthncrtr -m 0755 /srv/bazarr/config; note "created /srv/bazarr/config"; }
  }
  install_file "$REPO_ROOT/services/bazarr/docker-compose.yml" /srv/bazarr/docker-compose.yml
  (( DRY_RUN )) && return 0
  ensure_arrnet
  compose_up bazarr
  sleep 3
  verify_url https://bazarr.nthncrtr.com 200 || true
}
