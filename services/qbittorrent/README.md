...
| Max upload slots (global / per-torrent) | **100 / 16** | Defaults (20/4) starve a many-torrent seedbox. Per-torrent was 8 through 2026-05-23; bumped to 16 so when two fast leechers land on the same big remux each gets a healthier slice. Global 100 is still the binding cap. |
| BitTorrent protocol | **TCP only** (`bittorrent_protocol=1`) | Was `0` (TCP + uTP, qBit default). uTP is UDP-based + congestion-friendly; over Proton's WireGuard tunnel the micro-jitter triggers permanent uTP backoff, capping every uTP peer at single-digit KB/s. A/B measured 2026-05-23: aggregate jumped from <1 MB/s → 2-7 MB/s with no other change. Modern peers all support TCP fallback. |
| Peer TOS | **`0`** (no DSCP marking) | Was `1` (low-cost hint). Some upstream networks (including parts of Proton's egress path) interpret nonzero DSCP as "deprioritize." Setting to `0` requests best-effort. |
| Upload choking algorithm | **Anti-Leech** (`upload_choking_algorithm=2`) | Was `1` (Fastest Upload). Fastest Upload preferentially rewards peers we're already sending to fastest — wrong policy when bootstrapping from a 0.03-ratio cold start with no existing fast relationships. Anti-Leech distributes slots to leechers instead, building tit-for-tat reciprocity across the swarm. |
| Temp path (in-progress dir) | **`/incomplete`** (bind: `/srv/qbit-incomplete`) | In-progress pieces land on the SATA SSD instead of `/mnt/media` (USB HDD), which capped aggregate downloads at ~10 MB/s on small random writes regardless of network. Completed files move to `save_path` on `/mnt/media` (cross-fs copy, USB-HDD-bound). Caveat: `/srv` has ~90 GB free, so concurrent in-progress downloads are space-limited until the planned NVMe upgrade. *(The ~10 MB/s figure was measured on the exfat drive pre-2026-05-20; `/mnt/media` is ext4 now, so the in-progress-on-SSD decision still holds for the slow USB HDD but the exact number — echoed in `docker-compose.yml` and `apply-tuning.sh` comments — should be re-measured. Flagged, not re-tuned.)* |
| Disk cache | **1024 MB** | Default `-1` is ~64 MB. Holds hot pieces in RAM so popular seeds don't re-read from `/mnt/media` for every leecher request. Bumped from 512 → 1024 on 2026-05-23 — natto has 7 GB RAM and uses ~250 MB at idle, so the extra 512 MB is free. Safe because `vm.swappiness=10` (see `bootstrap/sysctl-natto-tuning.conf`) stops swap thrash. Revisit if RAM headroom shrinks. |
...
| DHT / PEX / LSD | **all off** (`dht=false`, `pex=false`, `lsd=false`) | Every torrent on this box comes from Orpheus or BHD (both private), and libtorrent honors each torrent's `private` flag to suppress these per-torrent. But: a single accidentally-added public torrent — drag-and-drop, paste a wrong URL — would happily enable all three with these globally on. Off globally removes the footgun; we lose nothing because no torrent here uses them. Bonus: kills the steady ~33% idle CPU spent maintaining ~370 DHT nodes (was a long-standing background load). |
| Send buffer watermarks | **`send_buffer_watermark=4000` KB, `send_buffer_low_watermark=200`, `send_buffer_watermark_factor=200`** | libtorrent defaults (500 / 10 / 50) were sized for early-2000s seedboxes — a single connection's outflight maxed at ~500 KB. With Proton's ~31 MB/s single-flow tunnel and a 30 MiB/s qBit upload cap, we never come close to using the link when a fast leecher does appear. Bumping the watermark + factor lets each connection have ~4 MB outflight, which a fast leecher can actually consume. Bounded by `max_uploads_per_torrent=16`. |
| OS socket buffer sizes | **`socket_send_buffer_size=1048576`, `socket_receive_buffer_size=1048576`** | Was `0` (= kernel default, ~200 KB). Explicit `SO_SNDBUF`/`SO_RCVBUF` lets TCP windows scale for the VPN's added RTT. Doesn't override the kernel's `tcp_wmem` ceiling, but lifts the floor — relevant because Proton's WG tunnel adds ~15-25 ms RTT vs direct, doubling the bandwidth-delay product. |
| File pool size | **256** | qBit caches open file handles for recently-accessed pieces. Default 100 was thrashing the LRU at 800+ torrents (peer asks for a piece whose file already got evicted → open/close churn). 256 covers normal working set without exhausting the host's fd limit (default 1024 per process). |
| Save resume data interval | **600 s** (10 min) | Was 60 s. Every 60 s qBit fsync'd resume data for 800+ torrents to `/config/qBittorrent/BT_backup/` — non-trivial disk-write penalty, especially during a bulk recheck batch. Worst-case loss on unclean crash with the new interval: ~10 min of newly-grabbed pieces' bookkeeping, all recoverable on next recheck. The actual file data is still fsync'd by libtorrent's piece-write path, independent of this knob. |
